...
function buildApp(options?: {
  ...
  createStaffCase?: IntakeRepository["createStaffCase"];
...
describe("staff create-and-submit an intake case (POST /internal/v1/intake-cases)", () => {
  it("denies the internal surface to a customer", async () => {
    const { app } = buildApp({ population: "customer", hasAdminRole: true });

    const response = await request(app).post("/internal/v1/intake-cases").send(validBody);
  ...
  it("rejects a submission with a duplicated evidence type", async () => {
    ...
    expect(response.status).toBe(422);
    expect(response.body.code).toBe("intake.duplicate_evidence_type");
    expect(createStaffCase).not.toHaveBeenCalled();
  });
  ...
  it("rejects a property value below the platform's minimum floor", async () => {
    ...
    expect(response.status).toBe(422);
    expect(response.body.code).toBe("intake.property_value_below_minimum");
    expect(createStaffCase).not.toHaveBeenCalled();
  });
  ...
  it("returns 404 when the applicant account id does not resolve to a real account", async () => {
    ...
    expect(response.body.code).toBe("intake.applicant_account_not_found");
  });
