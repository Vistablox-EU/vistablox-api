...
import { PrismaInvestorProfileKycProjectionRepository } from "../src/modules/investor-profile/repository/prisma-investor-profile-kyc-projection.repository.js";

const databaseUrl = process.env.TEST_DATABASE_URL;

describe.skipIf(databaseUrl === undefined)(
  "Investor-profile KYC eligibility projection PostgreSQL integration",
  () => {
    const authPool = new Pool({ connectionString: databaseUrl });
    const database = createPrismaClient(databaseUrl ?? "");
    let repository: PrismaInvestorProfileKycProjectionRepository;

    beforeAll(() => {
      repository = new PrismaInvestorProfileKycProjectionRepository(database);
    });
    ...
    it("reconcile only writes rows that actually differ from the source, and self-heals a manually-desynced row", async () => {
      ...
      try {
        ...
        await database.investorProfileKycEligibilityProjection.update({
          where: { accountId },
        ...
        await database.investorProfileKycEligibilityProjection.deleteMany({ where: { accountId } });
