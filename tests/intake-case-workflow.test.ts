...
const ownedCase: OwnedIntakeCase = {
  caseId: "case_01",
  stage: "draft",
...
function buildApp(options?: {
  caseRecord?: OwnedIntakeCase | null;
  listRows?: OwnedIntakeCase[];
...
describe("owner intake-case workflow", () => {
  it("lists only through the owner-scoped repository query", async () => {
    const { app, repository } = buildApp();

    const response = await request(app).get("/v1/intake-cases?limit=10");
  ...
  it("hides a case outside the caller's account scope as not found", async () => {
    ...
    const response = await request(app).get("/v1/intake-cases/case_other");
  ...
  it("rejects submission when mandatory initial evidence is absent", async () => {
    ...
    const response = await request(app)
      .post("/v1/intake-cases/case_01/submit")
      .send(body);

    expect(response.status).toBe(422);
    expect(response.body.code).toBe("intake.required_evidence_missing");
  ...
  it("rechecks KYC eligibility at submission time", async () => {
    ...
    expect(response.body.code).toBe("intake.submission_kyc_not_eligible");
  });
  ...
  it("rejects an already-submitted case", async () => {
    ...
    expect(response.body.code).toBe("intake.case_submission_conflict");
  });
...
describe("owner case messages (applicant lane only)", () => {
  it("reads the applicant lane on the owner's own case", async () => {
    ...
    const response = await request(app).get("/v1/intake-cases/case_01/messages");
  ...
  it("posts to the applicant lane as the owner, always — there is no other lane to choose", async () => {
    ...
    const response = await request(app)
      .post("/v1/intake-cases/case_01/messages")
