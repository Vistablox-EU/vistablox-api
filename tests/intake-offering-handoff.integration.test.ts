...
describe.skipIf(databaseUrl === undefined)(
  "intake approval to offering handoff PostgreSQL integration",
  () => {
    ...
    beforeAll(async () => {
      ...
      await database.intakeCase.update({
        where: { id: caseId },
    ...
    afterAll(async () => {
      ...
      // submission_revisions.case_id -> intake_cases and
      // submission_revisions.submitted_by_account_id -> account.accounts are
      // both ON DELETE RESTRICT, and intake_cases.property_id ->
    ...
    it("durably enqueues the offering handoff in the same transaction as approval, and the job opens a browsable offering", async () => {
      ...
      const recorded = await intakeRepository.recordFounderDecision({
        decision: "approve",
        accountId,
        caseId,
        traceId,
...
describe.skipIf(databaseUrl === undefined)(
  ...
  () => {
    ...
    beforeAll(async () => {
      boss = new PgBoss(databaseUrl ?? "");
      intakeRepository = new PrismaIntakeRepository(
        database,
        boss,
    ...
    it("getCaseForOperations returns the case's PIV's most recent offering via the same joined query, no separate round-trip", async () => {
      ...
      const result = await intakeRepository.getCaseForOperations(caseId);
    ...
    it("transitions a pre_offering_open case to post_ipo_structuring, and replaying the job is a safe no-op", async () => {
      ...
      const result = await intakeRepository.transitionToPostIpoStructuring({
        caseId,
        traceId,
      ...
      const updated = await database.intakeCase.findUniqueOrThrow({ where: { id: caseId } });
      expect(updated.stage).toBe("post_ipo_structuring");
      expect(
        await database.auditLog.count({
          where: { resourceId: caseId, action: "intake.case_post_ipo_structuring_started" },
        }),
      ).toBe(1);
      ...
      const replayed = await intakeRepository.transitionToPostIpoStructuring({
        caseId,
        traceId,
