...
import { PrismaOfferingKycProjectionRepository } from "../src/modules/offering/repository/prisma-offering-kyc-projection.repository.js";

const databaseUrl = process.env.TEST_DATABASE_URL;

describe.skipIf(databaseUrl === undefined)(
  "Offering KYC eligibility projection PostgreSQL integration",
  () => {
    const authPool = new Pool({ connectionString: databaseUrl });
    const database = createPrismaClient(databaseUrl ?? "");
    let repository: PrismaOfferingKycProjectionRepository;

    beforeAll(() => {
      repository = new PrismaOfferingKycProjectionRepository(database);
    });
    ...
    it("reconcile only writes rows that actually differ from the source, and self-heals a manually-desynced row", async () => {
      ...
      try {
        ...
        await database.offeringKycEligibilityProjection.update({
          where: { accountId },
        ...
        await database.offeringKycEligibilityProjection.deleteMany({ where: { accountId } });
