...
// One shared fixture (account + KYC + wallet + reservations + positions)
// backs all three repository classes below rather than being triplicated
// per module: they're three read/write surfaces over the same account-scoped
// rows, and building it three times over would only add duplicated setup
// code and slower CI for no extra coverage.
const databaseUrl = process.env.TEST_DATABASE_URL;

describe.skipIf(databaseUrl === undefined)("profile/wallet/investor-activity PostgreSQL integration", () => {
  ...
  let walletRepository: PrismaWalletRepository;
  let investorActivityRepository: PrismaInvestorActivityRepository;
  let profileRepository: PrismaProfileRepository;
  let accountPreferencesRepository: PrismaAccountPreferencesRepository;
  let accountClosureRepository: PrismaAccountClosureRepository;

  beforeAll(async () => {
    walletRepository = new PrismaWalletRepository(database);
    investorActivityRepository = new PrismaInvestorActivityRepository(database);
    accountPreferencesRepository = new PrismaAccountPreferencesRepository(database);
    accountClosureRepository = new PrismaAccountClosureRepository(database);
    profileRepository = new PrismaProfileRepository(
      database,
  ...
  afterAll(async () => {
    await database.accountClosureRequest.deleteMany({
      where: { accountId: { in: [accountId, otherAccountId] } },
    });
    await database.accountPreferences.deleteMany({
      where: { accountId: { in: [accountId, otherAccountId] } },
    });
  ...
  describe("PrismaProfileRepository", () => {
    it("reads the cross-schema profile aggregate without storing a duplicate profile", async () => {
      await expect(profileRepository.get(accountId)).resolves.toMatchObject({
        accountId,
  ...
  describe("PrismaAccountClosureRepository", () => {
    it("creates a pending request and surfaces it through the profile aggregate", async () => {
      const requestedAt = new Date("2026-09-05T09:00:00.000Z");
      const created = await accountClosureRepository.create({
        accountId,
      ...
      await expect(profileRepository.get(accountId)).resolves.toMatchObject({
        pendingClosureRequest: { reason: "Moving to a different platform", requestedAt },
      });

      const cancelled = await accountClosureRepository.cancel({
  ...
  describe("PrismaAccountPreferencesRepository", () => {
    it("seeds defaults on first write and merges a partial patch afterward", async () => {
      const seeded = await accountPreferencesRepository.update(otherAccountId, {
        marketingEmail: true,
      });
      ...
      const merged = await accountPreferencesRepository.update(otherAccountId, {
      ...
      await expect(profileRepository.get(otherAccountId)).resolves.toMatchObject({
        preferences: merged,
      });
    });
  });

  describe("PrismaInvestorActivityRepository", () => {
    it("paginates account-scoped reservations and returns only the latest money state", async () => {
      const firstPage = await investorActivityRepository.listReservations({
        accountId,
        limit: 2,
      });
      ...
      const secondPage = await investorActivityRepository.listReservations({
        accountId,
        limit: 2,
        after: {
    ...
    it("lists active positions before pending positions and excludes redeemed holdings", async () => {
      const activePage = await investorActivityRepository.listCurrentPositions({
        accountId,
        limit: 1,
      });
      ...
      const pendingPage = await investorActivityRepository.listCurrentPositions({
        accountId,
        limit: 2,
        after: {
    ...
    it("summarizes reservation and active-position counts", async () => {
      await expect(investorActivityRepository.getSummary(accountId)).resolves.toEqual({
        reservationCount: 3,
  ...
  describe("PrismaWalletRepository", () => {
    it("registers a wallet, replays idempotently, and rejects address conflicts", async () => {
      ...
      const created = await walletRepository.registerWallet({
        accountId: otherAccountId,
      ...
      const replayed = await walletRepository.registerWallet({
        accountId: otherAccountId,
        walletAddress: address,
      ...
      await expect(
        walletRepository.registerWallet({
          accountId: otherAccountId,
      ...
      await expect(walletRepository.getStatus(otherAccountId)).resolves.toMatchObject({
        registeredAt: null,
      });
    });
