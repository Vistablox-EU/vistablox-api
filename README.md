...
Reservation creation (`POST /v1/offerings/:offering_id/reservations`) is now implemented — an atomic, `FOR UPDATE`-locked capacity check (`AD-146`) alongside a Coinbase CDP onramp session created synchronously in the request path (`AD-255`, superseding the Stripe Onramp flow this line previously named as unsupported) — but the funding rail still defaults closed (`fundingRailAvailable = false` in both `GetInvestorOfferingService` and `CreateReservationService`), so every attempt still returns `409 funding_rail_unavailable` today. Opening it needs the still-unbuilt 15-minute unfunded-reservation expiry sweep and Coinbase transaction-status poller (no onramp/offramp webhook exists), plus live confirmation that Coinbase supports EUR/Base for this account. See [`docs/investor-offering.md`](docs/investor-offering.md#reservation-creation) and [`docs/coinbase-cdp-onramp.md`](docs/coinbase-cdp-onramp.md).
