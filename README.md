...
Reservation creation (`POST /v1/offerings/:offering_id/reservations`) is now implemented — an atomic, `FOR UPDATE`-locked capacity check (`AD-146`) alongside a Coinbase CDP onramp session created synchronously in the request path (`AD-255`, superseding the Stripe Onramp flow this line previously named as unsupported) — and the 15-minute unfunded-reservation expiry sweep the capacity-hold policy needs is also built (`case_timers.reservation_unfunded_expiry`, run every minute rather than hourly like this worker's other timers, since a 15-minute promise needs a tighter one). The funding rail still defaults closed regardless (`fundingRailAvailable = false` in both `GetInvestorOfferingService` and `CreateReservationService`), so every reservation attempt still returns `409 funding_rail_unavailable` today. Opening it needs a still-unbuilt Coinbase transaction-status poller (no onramp/offramp webhook exists) to advance `capital_state` past `eurc_purchase_pending`, plus live confirmation that Coinbase supports EUR/Base for this account. See [`docs/investor-offering.md`](docs/investor-offering.md#reservation-creation) and [`docs/coinbase-cdp-onramp.md`](docs/coinbase-cdp-onramp.md).
