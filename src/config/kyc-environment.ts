...
// Deliberately separate from environment.ts's loadEnvironment: this
// service's only job is receiving and processing the Didit webhook, so its
// Didit settings are required outright rather than optional-together (there
// is no "KYC disabled" mode here -- if this process is running, Didit must
// be configured), and it has none of the unrelated required vars
// (BETTER_AUTH_SECRET, SMTP_*, etc.) the shared schema forces on every
// consumer today.
