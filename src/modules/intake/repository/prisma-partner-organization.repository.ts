    ...
    return this.database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          ...
          action: "intake.legal_practice_created",
    ...
    return this.database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          id: `audit_${ulid()}`,
          actorAccountId: input.actorAccountId,
          action: "intake.legal_practice_status_updated",
          resourceType: "legal_practice",
          resourceId: practice.id,
          changes: {
            trace_id: input.traceId,
    ...
    return this.database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          id: `audit_${ulid()}`,
          actorAccountId: input.actorAccountId,
          action: "intake.appraisal_firm_created",
    ...
    return this.database.$transaction(async (transaction) => {
      ...
      await transaction.auditLog.create({
        data: {
          id: `audit_${ulid()}`,
          actorAccountId: input.actorAccountId,
          action: "intake.appraisal_firm_status_updated",
          resourceType: "appraisal_firm",
          resourceId: firm.id,
          changes: {
            trace_id: input.traceId,
