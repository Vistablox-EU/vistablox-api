...
import type { CaseMessageRecord, IntakeRepository, ThreadLane } from "../repository/intake.repository.js";
...
// repository methods below, matching how every other intake write
...
export class PostOwnCaseMessageService {
  ...
  public async execute(input: { accountId: string; caseId: string; body: string }) {
    const intakeCase = await this.repository.getOwnedCase(input.accountId, input.caseId);
    if (intakeCase === null) throw caseNotFoundError();
