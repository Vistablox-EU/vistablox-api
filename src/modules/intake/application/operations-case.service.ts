...
import type { GetKycDisplayProfileService } from "../../identity/application/kyc-display-profile.service.js";
...
import type {
  ...
  CreateStaffDraftSubmitBody,
} from "../api/intake-operations.schemas.js";
import { intakeCaseStageSchema } from "../api/intake.schemas.js";
import {
  addBusinessDays,
...
import {
  ...
  intakeEvidenceStatusLabels,
  intakeInformationRequestResolutionLabels,
  intakeInformationRequestStatusLabels,
...
export class GetCaseForOperationsService {
  public constructor(
    ...
    private readonly getKycDisplayProfile?: GetKycDisplayProfileService,
  ) {}

  public async execute(caseId: string) {
    const intakeCase = await this.repository.getCaseForOperations(caseId);
    if (intakeCase === null) throw caseNotFoundError();
    let identity = null as Awaited<ReturnType<GetKycDisplayProfileService["execute"]>>["data"];
    if (this.getKycDisplayProfile !== undefined) {
      try {
        identity = (await this.getKycDisplayProfile.execute(intakeCase.applicantAccountId)).data;
      } catch {
        // Identity enrichment is best-effort; a KYC provider/cache outage
        // must not make the operational case unavailable.
        identity = null;
      }
    }
    return toOperationsResponse(intakeCase, identity);
  }
}
...
export class PublishInformationRequestService {
  ...
  public async execute(input: {
    ...
    const [intakeCase, responseWindowDays] = await Promise.all([
    ...
    const decision = evaluateInformationRequestPublication({
      ...
      hasActiveRequest: intakeCase.informationRequests.some(
...
export class WithdrawInformationRequestService {
  ...
  public async execute(input: {
    ...
    const intakeCase = await this.repository.getCaseForOperations(input.caseId);
    if (intakeCase === null) throw caseNotFoundError();

    const request = intakeCase.informationRequests.find(
...
export class CloseCaseService {
  ...
  public async execute(input: {
    ...
    const intakeCase = await this.repository.getCaseForOperations(input.caseId);
    if (intakeCase === null) throw caseNotFoundError();
    if (!canCloseCase({ stage: intakeCase.stage, outcome: input.body.outcome })) {
...
// doesn't exist yet.
export class CreateStaffIntakeCaseService {
  public constructor(
    private readonly repository: IntakeRepository,
    private readonly clock: () => Date = () => new Date(),
  ) {}
  ...
  public async execute(input: {
    ...
    try {
      ...
      if (error instanceof ApplicantAccountNotFoundError) {
        throw new AppError({
          code: "intake.applicant_account_not_found",
...
/** Staff draft half of the staged upload workflow. */
export class CreateStaffDraftCaseService {
  public constructor(private readonly repository: IntakeRepository) {}
  public async execute(input: { body: Omit<CreateStaffCaseBody, "documents">; traceId: string }) {
    const b = input.body;
    const minimum = await this.repository.getMinimumPropertyValueEur();
    if (moneyToCents(b.property.owner_declared_value_eur) < moneyToCents(minimum)) {
      throw new AppError({ code: "intake.property_value_below_minimum", title: "Property value below minimum", status: 422, detail: `The owner-declared property value must be at least EUR ${minimum}.` });
    }
    const created = await this.repository.createDraftIntake({
      accountId: b.applicant_account_id,
      traceId: input.traceId,
      property: { countryCode: b.property.country_code, city: b.property.city, addressLine: b.property.address_line, landRegistryReference: b.property.land_registry_reference, latitude: null, longitude: null, ownerDeclaredValueEur: b.property.owner_declared_value_eur, hasExistingEncumbrance: b.property.has_existing_encumbrance, residentialSubtype: b.property.residential_subtype, livingAreaSqM: b.property.living_area_sq_m, bedrooms: b.property.bedrooms, bathrooms: b.property.bathrooms, floor: b.property.floor, totalFloors: b.property.total_floors, yearBuilt: b.property.year_built, condition: b.property.condition, energyRating: b.property.energy_rating, rooms: b.property.rooms.map((r) => ({ roomType: r.room_type, sizeSqM: r.size_sq_m })) },
    });
    return { data: { case_id: created.caseId, property_id: created.propertyId, stage: created.stage, applicant_account_id: b.applicant_account_id } };
  }
}

export class SubmitStaffDraftCaseService {
  public constructor(private readonly repository: IntakeRepository) {}
  public async execute(input: { caseId: string; traceId: string; body: CreateStaffDraftSubmitBody }) {
    const detail = await this.repository.getCaseForOperations(input.caseId);
    if (detail === null) throw new AppError({ code: "intake.case_not_found", title: "Case not found", status: 404, detail: "The intake case does not exist." });
    const result = await this.repository.submitInitialCase({ accountId: detail.applicantAccountId, caseId: input.caseId, traceId: input.traceId, submissionData: input.body.submission_data, documents: input.body.documents.map((d) => ({ documentType: d.document_type, documentRef: d.document_ref, extractDated: d.extract_dated === null ? null : new Date(d.extract_dated) })) });
    if (result === null) throw new AppError({ code: "intake.case_not_submittable", title: "Case cannot be submitted", status: 409, detail: "The case is no longer in a draft state." });
    return { data: { case_id: result.caseId, revision_id: result.revisionId, revision_number: result.revisionNumber, stage: result.stage, submitted_at: result.submittedAt.toISOString() } };
  }
}
...
export class SendManualReminderService {
  ...
  public async execute(input: { caseId: string; requestId: string }) {
    ...
    if (request.applicantContactEmail === null) {
      throw new AppError({
        code: "intake.applicant_contact_email_missing",
...
async function resolvePublishedInformationRequestOrThrow(
  ...
  const intakeCase = await repository.getCaseForOperations(caseId);
  if (intakeCase === null) throw caseNotFoundError();
  const exists = intakeCase.informationRequests.some(
...
export class RetryPostIpoStructuringHandoffService {
  ...
  public async execute(input: { caseId: string; traceId: string }) {
    const intakeCase = await this.repository.getCaseForOperations(input.caseId);
    if (intakeCase === null) throw caseNotFoundError();
    if (intakeCase.offering === null || intakeCase.offering.finalOfferingPublishedAt === null) {
...
function toOperationsResponse(
  input: OperationsCaseDetail,
  identity: {
    given_name: string;
    family_name: string;
  ...
  return {
    ...
    applicant: {
      account_id: input.applicantAccountId,
      display_name: identity?.full_display_name ?? null,
      given_name: identity?.given_name ?? null,
      family_name: identity?.family_name ?? null,
      id_document_country_code: identity?.id_document_country_code ?? null,
      identity_available: identity !== null,
      profile_synced_at: identity?.synced_at ?? null,
    },
    ...
    offering:
      input.offering === null
        ...
        : {
            ...
            status_label: displayLabel(input.offering.status, offeringStatusLabels)!,
      ...
      input.submission === null
        ...
        : {
            ...
            evidence: input.submission.evidence.map((evidence) => ({
              ...
              document_type_label: displayLabel(evidence.documentType, intakeDocumentTypeLabels)!,
              status: evidence.status,
              status_label: displayLabel(evidence.status, intakeEvidenceStatusLabels)!,
...
export function toInformationRequestResponse(input: InformationRequestRecord) {
  return {
    request_id: input.requestId,
    status: input.status,
    status_label: displayLabel(input.status, intakeInformationRequestStatusLabels)!,
    ...
    resolution_type_label: displayLabel(input.resolutionType, intakeInformationRequestResolutionLabels),
...
function reviewConflictError(action: string, cause?: unknown): AppError {
  return new AppError({
    code: "intake.review_transition_conflict",
...
function postIpoHandoffNotReadyError(): AppError {
  return new AppError({
    code: "intake.post_ipo_handoff_not_ready",
...
function evidenceNotFoundError(): AppError {
  return new AppError({
    code: "intake.evidence_not_found",
...
function evidenceCaseMismatchError(cause?: unknown): AppError {
  return new AppError({
    code: "intake.evidence_case_mismatch",
...
function partnerOrganizationNotFoundError(
  ...
  return new AppError({
    code: `intake.${resourceType}_not_found`,
...
function submissionPolicyError(
  ...
  return new AppError({
    code: "intake.case_submission_conflict",
...
function propertyValueBelowMinimumError(minimumValue: string): AppError {
  return new AppError({
    code: "intake.property_value_below_minimum",
...
function partnerOrganizationNotActiveError(
  ...
  return new AppError({
    code: `intake.${resourceType}_not_active`,
