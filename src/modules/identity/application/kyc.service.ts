...
export class ProcessDiditWebhookService {
  ...
  public async execute(input: {
    ...
    const target = await this.repository.findByDiditReference(input.sessionId);
    if (target === null) {
      await this.repository.recordUnmatchedProviderEvent({
        eventKey,
        eventId: input.eventId,
    ...
    if (
      ...
      (decision.sessionId !== input.sessionId ||
        ...
        (decision.workflowId !== null &&
          decision.workflowId !== this.configuration.workflowId) ||
    ...
    const outcome = evaluateDiditOutcome({
      status,
      decision,
    ...
    const result = await this.repository.applyProviderOutcome({
      eventKey,
      eventId: input.eventId,
      diditReference: input.sessionId,
