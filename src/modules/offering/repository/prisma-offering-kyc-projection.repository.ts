...
// Phase 7: offering's own event-driven local copy of KycEligibility --
// mirrors PrismaOriginationKycProjectionRepository exactly (see its own
// comments for the full reasoning); the one thing that differs for this
// consumer specifically is worker.ts's reconcile cadence (5 minutes, not
// hourly) -- this is the one projection on CreateReservationService's path.
export class PrismaOfferingKycProjectionRepository implements KycEligibilityReader {
  public constructor(private readonly database: DatabaseClient) {}

  public async getEligibilitySnapshot(
    accountId: string,
  ): Promise<KycEligibilitySnapshot | null> {
    return this.database.offeringKycEligibilityProjection.findUnique({
      where: { accountId },
  ...
  public async applyEvent(snapshot: KycEligibilitySnapshot): Promise<JobRunSummary> {
    ...
    await this.database.offeringKycEligibilityProjection.upsert({
      where: { accountId },
  ...
  public async reconcile(): Promise<JobRunSummary> {
    const [sourceRows, projectionRows] = await Promise.all([
      ...
      this.database.offeringKycEligibilityProjection.findMany({ select: SNAPSHOT_SELECT }),
    ]);
