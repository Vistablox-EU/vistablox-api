...
export function createTotpRouter(
  ...
  router.post("/verify", requireAuthentication, async (request, response) => {
    ...
    const result = await verifyTotp.execute(context.accountId, body.code, context.providerSessionId);
    response.json(
...
function requireCustomerContext(
  ...
  return { accountId: context.accountId, providerSessionId: context.providerSessionId };
}
